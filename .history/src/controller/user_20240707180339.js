...
async function update(req, res) {
  try {
    ...
    if (paramsId === null || paramsId === undefined)
      return res.status(httpStatus.BAD_REQUEST).send(`Id: ${paramsId} Erro durante requisição ${req.error}`)


    if (tNumber.isInteger(Number.parseInt(paramsId)))
      return res
        .status(httpStatus.BAD_REQUEST)
        .send(`Id: ${paramsId} - typeof: ${Number.isInteger(paramsId)} Id enviado na requisição é inválido`)
