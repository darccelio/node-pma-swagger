...
async function update(req, res) {
  try {
    ...
    if(paramsId === null || paramsId === undefined)
      return res.status(httpStatus.BAD_REQUEST).send("Erro durante requisição");

    if(typeof(paramsId) != Number)
      return res.status(httpStatus.BAD_REQUEST).send("Id enviado na requisição é inválido");
